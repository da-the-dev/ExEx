...
export default class ProfileService {
    ...
    static async enableProfile(profileName: string, ctx: vscode.ExtensionContext) {
        ...
        StorageService.setDeepWorkspaceKey<DeepExtension[]>('extensionsIdentifiers/enabled', profile.enabledExtensions.map(e => new Extension(e.name, e.id, e.uuid).toDeepExtension()), ctx)
        StorageService.setDeepWorkspaceKey<DeepExtension[]>('extensionsIdentifiers/disabled', profile.disabledExtensions.map(e => new Extension(e.name, e.id, e.uuid).toDeepExtension()), ctx)
